...
struct Nodo {
	...
	struct Nodo* anterior; // Nuevo puntero para el nodo anterior
};
...
void insertarLista(Nodo*& lista, int n) {
	...
	Nodo* aux1 = lista;
	Nodo* aux2 = NULL; // Nuevo puntero auxiliar para el nodo anterior
	...
	if (lista == aux1) {
		...
		nuevo_nodo->anterior = aux2; // Establecer enlace al nodo anterior
	}
	if (aux1 != NULL) {
		aux1->anterior = nuevo_nodo; // Establecer enlace al nodo siguiente
	}
	nuevo_nodo->siguiente = aux1;
	printf("\nElemento %d insertado correctamente en la lista", n);
}
